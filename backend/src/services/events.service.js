...
const createEvent = async ({ eventName, id_category, description, location, date_time, image_url, ticketTypes }) => {
...
const updateEvent = async (id, { eventName, id_category, description, location, date_time, image_url }) => {
  ...
  if (image_url !== undefined) {
    await prisma.eventImage.deleteMany({ where: { id_event: Number(id), type: 'poster' } });
    if (image_url.trim() !== '') {
...
const registerInterest = async (eventId, userId) => {
  ...
  const total = await prisma.interest.count({ where: { id_event: Number(eventId) } });
  invalidateEventCache();
  return total;
};
