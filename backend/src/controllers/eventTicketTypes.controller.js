...
const getEventTicketTypes = async (req, res) => {
  try {
    const data = await eventTicketTypesService.getTicketTypesByEvent(req.params.id);
...
const createEventTicketType = async (req, res) => {
  try {
    const data = await eventTicketTypesService.createTicketType(req.params.id, req.body);
...
const updateEventTicketType = async (req, res) => {
  try {
    const data = await eventTicketTypesService.updateTicketType(req.params.id, req.params.id_ticket, req.body);
    return res.json(data);
  } catch (err) {
    if (err.code === 'CAPACITY_CONFLICT') return res.status(409).json({ error: err.message });
    console.error(err);
...
const softDeleteEventTicketType = async (req, res) => {
  try {
    await eventTicketTypesService.deleteTicketType(req.params.id, req.params.id_ticket);
    return res.json({ message: 'Ticket type deleted successfully' });
  } catch (err) {
    if (err.code === 'HAS_SALES') return res.status(409).json({ error: err.message });
    if (err.message === 'TICKET_TYPE_NOT_FOUND') return res.status(404).json({ error: 'Ticket type not found' });
    console.error(err);
