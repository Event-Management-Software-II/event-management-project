      ...
      <template v-else-if="ticketTypes.length">
        <div class="footer-types">
          ...
          >
            ...
            <span class="btn-type-price">{{
              tp.price === 0 ? 'Gratis' : `$${tp.price.toLocaleString('es-CO')}`
            }}</span>
          </button>
          <button
            v-if="ticketTypes.length > 3"
            class="btn-type btn-type--more"
            @click="openDetail"
          >
  ...
  if (isAuthenticated.value) {
    try {
      const res = await fetch(
        `${API}/favorites/${props.event.id_event}/status`,
        {
          headers: authHeaders(),
        }
      );
...
const categoryGradient = computed(
  () =>
    gradients[props.event.categoryName ?? ''] ??
