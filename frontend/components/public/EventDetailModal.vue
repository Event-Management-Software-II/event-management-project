        ...
        <div class="modal-left">
          ...
          <div class="modal-img-wrap">
            ...
            <span
              ...
              :class="isActive ? 'chip--active' : 'chip--past'"
            >
              ...
              <div class="venue-zones">
                <div
                  ...
                  :class="[
                    `zone-color-${i % 6}`,
                    {
                      'zone--selected': selectedTypeId === tp.id,
                      'zone--sold': (availability[tp.id] ?? 0) === 0,
                      'zone--disabled': !isActive,
                    },
                  ]"
                ...
                >
                  ...
                  <span class="zone-seats">{{
                    (availability[tp.id] ?? 0) === 0
                      ? 'Agotado'
                      : `${availability[tp.id]} disp.`
                  }}</span>
                </div>
              </div>
              ...
              <div v-if="step === 'select'">
                ...
                <div class="types-list">
                  <div
                    ...
                    class="type-item"
                    :class="{
                      'type-item--selected': selectedTypeId === tp.id,
                      'type-item--sold': (availability[tp.id] ?? 0) === 0,
                    }"
                  ...
                  >
                    ...
                    <div class="type-item-right">
                      <span
                        class="type-avail"
                        :class="{
                          'sold-out': (availability[tp.id] ?? 0) === 0,
                        }"
                      >
                        {{
                          (availability[tp.id] ?? 0) === 0
                      ...
                      <button class="btn-add" @click="addToCart">
                        ...
                        <span class="add-price">{{
                          selectedType.price === 0
                            ? 'Gratis'
                            : `$${(selectedType.price * qty).toLocaleString('es-CO')}`
                        }}</span>
                      </button>
                    </div>
                  </div>
              ...
              <div v-if="step === 'confirm'">
                ...
                <div class="confirm-summary">
                  <div class="confirm-event">
                    <span class="confirm-event-name">{{
                      event.NameEvent
                    }}</span>
                    <span class="confirm-event-date">{{
                      formatDate(event.date_time)
                    }}</span>
                  </div>
                  <div
                ...
                <!-- Botón comprar más -->
                <button
                  class="btn-secondary btn-buy-more"
                  @click="step = 'select'"
                >
                  + Comprar más boletas
                </button>
...
const selectedType = computed(
  () => ticketTypes.value.find((t) => t.id === selectedTypeId.value) ?? null
);
...
async function confirmPurchase() {
  ...
  const result = await purchaseTickets(
    {
      id_event: props.event.id_event,
      NameEvent: props.event.eventName ?? props.event.NameEvent,
  ...
  if (!result.success) {
    buyError.value = result.error ?? 'Error al procesar.';
    return;
  }
...
function defaultDescription(tp: TicketType): string {
  const priceStr =
    tp.price === 0
      ? 'entrada gratuita'
      : `entrada a $${tp.price.toLocaleString('es-CO')}`;
...
const categoryGradient = computed(
  () =>
    gradients[props.event.nameCategory] ??
...
.type-item:hover:not(.type-item--sold) {
  border-color: #34656d;
  background: #f0faf8;
}
.type-item--selected {
