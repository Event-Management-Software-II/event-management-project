  ...
  <div>
    ...
    <div class="filters">
      <div class="search-box">
        ...
        <input v-model="searchQuery" placeholder="Buscar tipo…" class="search-input"/>
      </div>
      ...
      <div class="table-scroll">
        <table class="admin-table">
          ...
          <tbody>
            ...
            <tr v-for="item in filteredCatalog" :key="item.id_catalog" :class="{ 'row-inactive': item.deleted_at }">
              <td>
                <div class="cat-name-cell">
                  <div class="cat-bar" :class="{ 'cat-bar--inactive': item.deleted_at }"></div>
                  <span class="cat-name">{{ item.typeName }}</span>
                </div>
              </td>
              <td>
                <span :class="['badge', item.deleted_at ? 'badge--hidden' : 'badge--visible']">
                  {{ item.deleted_at ? 'Inactivo' : 'Activo' }}
                </span>
              </td>
              <td>
                <div class="actions">
                  <AppButtonAdmin variant="ghost" size="sm" icon-only @click="openModal('edit', item)" title="Editar">
                    <template #icon>
                  ...
                  <AppButtonAdmin
                    ...
                    @click="tryDeactivate(item)" title="Inactivar"
                  >
                    <template #icon>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
                  ...
                  <AppButtonAdmin
                    ...
                    @click="handleReactivate(item.id_catalog)" title="Restaurar"
                  >
                    <template #icon>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
        ...
        <div v-if="modalOpen" class="modal-overlay" @click.self="closeModal">
          <div class="modal-box" style="max-width:440px">
            <div class="modal-header">
              <h2 class="modal-title">{{ mode === 'create' ? 'Nuevo tipo de ticket' : 'Editar tipo de ticket' }}</h2>
...
const {
  ...
  deactivateCatalogItem, reactivateCatalogItem,
} = useTicketCatalog()
...
const filteredCatalog = computed(() => {
  const list = sortAsc.value
    ...
    : [...catalog.value].sort((a, b) => b.typeName.localeCompare(a.typeName, 'es'))
  if (!searchQuery.value) return list
  return list.filter(i => i.typeName.toLowerCase().includes(searchQuery.value.toLowerCase()))
})
...
async function save() {
  ...
  else formError.value = result.errors?.typeName ?? result.message ?? 'Error desconocido.'
  saving.value = false
}
...
async function tryDeactivate(item: TicketCatalog) {
  ...
  if (!result.success) {
    deactivateErrorMsg.value  = result.message ?? 'No es posible inactivar este tipo.'
...
.cat-bar--inactive { background:var(--border-strong); }
