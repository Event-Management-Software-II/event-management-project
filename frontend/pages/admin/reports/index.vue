      ...
      <div class="table-scroll">
        <table class="admin-table">
          ...
          <tbody>
            ...
            <tr
              v-for="(row, i) in salesReport"
              :key="`${row.id_event}-${row.ticket_type_name}`"
            >
              <td class="td-rank">
                <span
                  :class="[
              ...
              <td style="font-size: 0.85rem">{{ row.ticket_type_name }}</td>
              <td class="td-count">
                <div class="bar-wrap">
                  <div
                    class="bar"
                    :style="{ width: salesBarWidth(row.tickets_sold) + '%' }"
                  ></div>
...
const maxSales = computed(() =>
  salesReport.value.length
    ? Math.max(...salesReport.value.map((r) => r.tickets_sold))
    : 1
);
