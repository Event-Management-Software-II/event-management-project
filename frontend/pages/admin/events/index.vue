            ...
            >
              ...
              <td>
                <span
                  :class="[
                    'badge',
                    ev.deleted_at ? 'badge--hidden' : 'badge--visible',
                  ]"
                >
          ...
          <div class="modal-box">
            ...
            <div class="modal-body">
              ...
              <AppInputAdmin
                ...
                label="Descripción"
                as="textarea"
                placeholder="Descripción del evento (mín. 20 caracteres)"
                ...
                <div class="ticket-toggle">
                  ...
                  >
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
                      ...
                      />
                      <path
                        d="M12 6v1m0 10v1M9 9.5a3 3 0 015.5 1.5c0 2-3 2.5-3 4.5"
                        stroke="currentColor"
                        stroke-width="2"
                        stroke-linecap="round"
                      />
                  ...
                  >
                    ...
                    <div class="ticket-type-fields">
                      <div class="field-group">
                        ...
                        <span
                          v-if="formErrors[`tt_${idx}_id_catalog`]"
...
const filteredEvents = computed(() => {
  return events.value.filter((ev) => {
    const matchName =
      !searchQuery.value ||
      ev.eventName.toLowerCase().includes(searchQuery.value.toLowerCase());
    const matchCat =
      !filterCategory.value || ev.id_category === filterCategory.value;
...
function availableCatalogFor(idx: number) {
  const selectedInOtherRows = form.value.ticketTypes
    ...
    .filter(Boolean);
  return ticketCatalog.value.filter(
    (cat) => !selectedInOtherRows.includes(String(cat.id_catalog))
  );
}
...
function openModal(m: 'create' | 'edit', ev?: Event) {
  ...
  if (ev) {
    const existingTickets: TicketTypeRow[] = (ev.ticketTypes ?? []).map(
      (tt: any) => ({
...
function confirmDelete(ev: Event) {
