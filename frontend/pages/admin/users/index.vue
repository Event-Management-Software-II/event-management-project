  ...
  <div>
    ...
    <div class="filters">
      <div class="search-box">
        ...
        <input v-model="searchQuery" placeholder="Buscar por nombre o correo…" class="search-input"/>
      </div>
    ...
    <div class="card">
      <div class="table-scroll">
        <table class="admin-table">
          ...
          <tbody>
            <tr v-if="loading">
              <td colspan="4" style="text-align:center;padding:32px;color:var(--text-muted)">Cargando…</td>
            </tr>
            <tr v-else-if="error">
              <td colspan="4" style="text-align:center;padding:32px;color:var(--danger)">{{ error }}</td>
            </tr>
            ...
            <tr v-for="u in filteredUsers" :key="u.id">
              ...
              <td>
                <span :class="['role-badge', u.role === 'admin' ? 'role-badge--admin' : 'role-badge--external']">
...
.user-avatar { width:30px; height:30px; border-radius:50%; background:var(--primary-pale); color:var(--primary); font-size:.7rem; font-weight:800; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
.user-name { font-weight:600; font-size:.88rem; color:var(--text-primary); }
.td-secondary { font-size:.84rem; color:var(--text-secondary); }
.role-badge { display:inline-block; padding:3px 10px; border-radius:20px; font-size:.7rem; font-weight:700; letter-spacing:.04em; }
.role-badge--admin    { background:var(--primary-pale); color:var(--primary); }
.role-badge--external { background:var(--bg-elevated); color:var(--text-muted); border:1px solid var(--border); }
.table-footer { padding:12px 16px; font-size:.78rem; color:var(--text-muted); border-top:1px solid var(--border); text-align:right; }
</style>